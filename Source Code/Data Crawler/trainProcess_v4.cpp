...
#include <curl/easy.h> // 新增：用于 connshut 等底层操作
...
void process() {
    ...
    for (int i = start_index; i <= id; i++) {
        ...
        for (int k = 1; k <= 3 && (model == "common"); k++) {
            cout << "Retry " << k << " for " << trInfo[i][2] << " [Connection Reset]" << endl;
            
            // 关键优化：在重试前重置连接
            // 1. 尝试关闭现有连接（模拟断网重连）
            curl_easy_setopt(curl, CURLOPT_CONNECTTIMEOUT, 10L); 
            // curl_easy_cleanup 和 curl_easy_init 成本太高，这里使用 connshut (如果可用) 或单纯 reset
            // 实际上 curl_easy_reset 不会断开连接，最好的方法是强制关闭连接
            // 但在 easy interface 中，最简单的“重置”方法是清空状态并等待新IP（如果你手动换了的话）
            
            // 2. 睡眠等待 (给你手动切换网络的时间，或者让系统释放僵死连接)
        ...
        // 三次 Retry 失败后的处理
        if (model == "common") {
            cout << "!!! Change WLAN !!!" << endl;
            cout << "!!! Remember (" << i << ") Exit" << endl;
